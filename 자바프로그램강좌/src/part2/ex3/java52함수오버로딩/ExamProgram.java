...
package part2.ex3.java52함수오버로딩;
...
//	exams[2] = new Exam();
		...
		// 배열 참조변수(exams) 선언 : sub class에서 공유 가능
		...
		while (keepLoop) {
			 ...
			 switch (menu) {
			 		...
			 		case 1:
			 			...
			 			inputList(exams, current);		// 배열 참조변수(exams) 주소 전달 (stack), 
			 			break;
					case 2:
	...
	private static void inputList(Exam[] exams, int current) {
		...
		// 학생은 몇명이 입력될지 모름
				...
				exams[current] = exam;			//heap값의 주소를 stack으로 전달
				current++;
